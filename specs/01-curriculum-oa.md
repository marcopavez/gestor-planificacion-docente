...
> **Deriva de:** la decisión del dueño (2026-06-07) de **eliminar RAG/normativa**; `docs/bases-curriculares-primera-a-sexto-basico.pdf` como fuente; el corpus ya curado `corpus/curriculum/matematica-1-basico.json`; ADR-004 (corpus versionado).
...
### 2.1 Entra
- **Extracción + curación** de los OA de las Bases Curriculares para 1º–6º básico, todas las asignaturas del MVP (`[VERIFICAR]` lista exacta — README §6 #4).
...
| Currículum de 7º básico en adelante (media) | Post-MVP (MVP = 1º–6º) |
...
- **RF-1.2 · Cobertura del MVP.** Existe un archivo por cada `(asignatura, nivel)` del MVP, 1º a 6º. Un **manifiesto** (`corpus/curriculum/_manifest.json`) enumera lo disponible y su `corpus_version`. *(Decisión dueño: todas las asignaturas)*
...
- **H-1.5** `chore(corpus): poblar 1º–6º para las asignaturas del MVP` — datos (iterativo por asignatura).

---

## 6. Criterios de aceptación (CA-1.n)

- **CA-1.1** Para cada `(asignatura, nivel)` del MVP existe un JSON válido que pasa la validación de integridad.
...
- **#4 (README):** lista exacta de asignaturas del MVP — `[VERIFICAR]` con el dueño.
