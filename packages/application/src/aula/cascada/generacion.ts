...
export const INSTR_GUIA = instruccion(
  [
    ...
    "- 'ejercicios': práctica graduada (recordar → aplicar). Tipos: 'seleccion_multiple', 'verdadero_falso', 'completacion', 'desarrollo', 'ordenar' (con 'secuencia_correcta'), 'terminos_pareados' (con 'pares' columnaA↔columnaB) o 'pictorico'. Selección múltiple y verdadero/falso con EXACTAMENTE una alternativa correcta.",
    "- En un ítem 'pictorico', 'imagen' = una DESCRIPCIÓN visual CONCRETA anclada al conocimiento (qué se ve, p. ej. 'tres semillas germinando en un vaso'); NO es una imagen real, es la descripción con la que se genera una ilustración line-art (observa-y-responde). Incluye AL MENOS UN ejercicio 'pictorico' que apoye el aprendizaje.",
