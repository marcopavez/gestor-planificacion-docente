// GET /api/aula/guia/[jobId] — estado del job de guía para el polling (Tanda 1). Mientras no esté
// 'hecho' devuelve solo {estado, intentos, error}. Hecho → lee el documento borrador persistido y
// devuelve su contenido (la Guía), gates, estado de revisión y autor (lo que la pantalla HIL necesita).
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      contenido: doc.contenido, // la Guía (borrador editable en HIL)
      resultadoGates: doc.resultadoGates,
    });
  } catch (e) {
    return responderError500(log, e, { jobId }, 'GET /guia/[jobId] falló');
  }
}
