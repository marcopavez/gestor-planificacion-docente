// POST /api/aula/revision/[id]/rechazar — transición HIL en_revision → rechazado (RF-PA.11).
...
const log = crearLoggerHijo('web/revision/rechazar');
...
export async function POST(
  ...
  try {
    ...
    const resultado = await revisar.rechazar(id);
    ...
    // 'rechazar' no aprueba: la única regla posible aquí es 'transicion_invalida' → 409.
    return NextResponse.json(
      { error: resultado.mensaje, regla: resultado.regla },
      { status: 409 },
    );
  } catch (e) {
    const mensaje = e instanceof Error ? e.message : 'Error al rechazar el documento.';
    log.error({ err: mensaje, id }, 'POST /revision/[id]/rechazar falló');
