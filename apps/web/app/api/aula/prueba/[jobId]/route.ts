// GET /api/aula/prueba/[jobId] — estado del job de prueba para el polling (Fase 4). Mientras no esté
// 'hecho' devuelve solo {estado, intentos, error}. Hecho → lee el documento borrador persistido y
// devuelve su contenido (la Prueba), gates, estado de revisión y autor (lo que la pantalla HIL necesita).
...
const log = crearLoggerHijo('web/prueba/estado');
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      contenido: doc.contenido, // la Prueba (borrador editable en HIL)
      resultadoGates: doc.resultadoGates,
    });
  } catch (e) {
    return responderError500(log, e, { jobId }, 'GET /prueba/[jobId] falló');
  }
}
